...
class HistoricalTradingManager(TradingManager):
    def __init__(self, instruments: dict[str, Strategy], chunkSize: int, dataSources: dict[str, DataSource], initialAmount: float, startDate: dt):
        ...
        for instrument in instruments:
            ...
            lastCandle = dataSources[instrument].getSlice(dt.fromtimestamp(0), startDate)[-1]
            exchangeRates[instrument] = lastCandle.close
            self._currentCandles[instrument] = lastCandle
        self.virtualPortfolio = VirtualPortfolio(initialAmount, lotSizes, exchangeRates, startDate)
    ...
    def start(self):
        ...
        while continueCount != len(self.instruments):
            ...
            for instrument in self.instruments:
                if self.dataSources[instrument].nextCandle(self._currentCandles[instrument]) is not None:
                    quantity = self.instruments[instrument].predict(self.dataSources[instrument].getSlice(dt.fromtimestamp(0), self._currentCandles[instrument].datetime)[:-self.chunkSize - 1: -1][::-1])
