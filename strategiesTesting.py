...
trainingUseCase = PredictorTrainingUseCase(trainingDataSource, mlPredictor, trainingUseCaseLogger)
trainingUseCase.execute()
algoPredictor = IndicatorPredictor(14)
...
testingUseCase = StrategyTestingUseCase(testingDataSource, dummyPredictor, strategy, assessor, logger, dummyExecutor, 100)
...
plt.legend()
plt.tight_layout()
plt.savefig('strategyResult/dummyVolatilityThresholdGraph.png', dpi=300, bbox_inches='tight')
...
testingUseCase = StrategyTestingUseCase(testingDataSource, algoPredictor, strategy, assessor, logger, algoExecutor, 100)
testingUseCase.execute()
print(len(capitalizationData.cap))
percentages = [ (i / 100000) * 100 for i in capitalizationData.cap]
iters = [i for i in range(1, len(capitalizationData.cap) + 1)]
...
plt.xlabel('Итерация', fontsize=12)
plt.ylabel('Процент от исходного капитала', fontsize=12)
plt.title('Стратегия ожидаемая волатильность и алгоритмический предсказатель', fontsize=14)
plt.grid(True, linestyle='--', alpha=0.7)
plt.axhline(y=100, color='r', linestyle='-', alpha=0.3, label='100% (100000)')
...
plt.legend()
plt.tight_layout()
plt.savefig('strategyResult/algoVolatilityThresholdGraph.png', dpi=300, bbox_inches='tight')

print("==================Ml predictor==================")
capitalizationData.cap.clear()
assessor = VolatilityThresholdAssessor(mlPortfolio, mlContextProvider, 0.5)
testingUseCase = StrategyTestingUseCase(mlTestingDataSource, mlPredictor, strategy, assessor, logger, mlExecutor, 100)
testingUseCase.execute()
print(len(capitalizationData.cap))
percentages = [ (i / 100000) * 100 for i in capitalizationData.cap]
iters = [i for i in range(1, len(capitalizationData.cap) + 1)]
...
plt.xlabel('Итерация', fontsize=12)
plt.ylabel('Процент от исходного капитала', fontsize=12)
plt.title('Стратегия ожидаемая волатильность и предсказатель на основе машинного обучения', fontsize=14)
plt.grid(True, linestyle='--', alpha=0.7)
plt.axhline(y=100, color='r', linestyle='-', alpha=0.3, label='100% (100000)')
...
plt.legend()
plt.tight_layout()
plt.savefig('strategyResult/mlVolatilityThresholdGraph.png', dpi=300, bbox_inches='tight')